...
from polychat.keys.keychain import load_from_keychain, store_in_keychain


def test_load_from_keychain_success():
    """Test loading API key from keychain."""
    with patch('polychat.keys.keychain.keyring') as mock_keyring:
...
def test_load_from_keychain_not_installed():
    ...
    with patch('polychat.keys.keychain.keyring', None):
