...
please implement a feature to show estimated costs in us dollars after each response. models.py should now have cost data. output/input costs per million tokens. can we safely implement cache-hit related calculations too? when this is implemented, we need to also document that displayed costs are merely estimates. it would be safer if cost related data too is displayed each time. if user finds something significantly inaccurate, they might contact me. when working on this, please find my playbook and follow my preferences.
