...
class MetadataInspectionCommandHandlers:
    ...
    async def show_status(self, args: str) -> str:
        ...
        output = [
            ...
            "",
            "Providers",
            f"Assistant: {self._deps.manager.assistant.display()}",
            f"Helper:    {self._deps.manager.helper.display()}",
            "",
            "Prompts",
            ...
            f"Retry:     {'ON' if self._deps.manager.retry.active else 'OFF'}",
            f"Secret:    {'ON' if self._deps.manager.secret.active else 'OFF'}",
