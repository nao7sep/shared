...
def test_load_from_keychain_success():
    ...
    with patch('poly_chat.keys.keychain.keyring') as mock_keyring:
        mock_keyring.get_password.return_value = "sk-test-keychain-key"

        key = load_from_keychain("poly-chat", "test-account")

        assert key == "sk-test-keychain-key"
        mock_keyring.get_password.assert_called_once_with("poly-chat", "test-account")
...
def test_store_in_keychain_success():
    ...
    with patch('poly_chat.keys.keychain.keyring') as mock_keyring:
        store_in_keychain("poly-chat", "test-account", "sk-new-key")

        mock_keyring.set_password.assert_called_once_with(
            "poly-chat", "test-account", "sk-new-key"
        )
...
def test_store_in_keychain_not_installed():
    ...
    with patch('poly_chat.keys.keychain.keyring', None):
        with pytest.raises(ImportError, match="keyring package not installed"):
            store_in_keychain("poly-chat", "test-account", "sk-key")
