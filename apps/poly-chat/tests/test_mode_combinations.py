"""Tests for secret mode + search mode + thinking mode combinations."""
...
def test_session_manager_modes_independent(session_manager):
    ...
    assert session_manager.thinking_mode is False
    ...
    # Enable only thinking
    session_manager.secret_mode = False
    session_manager.thinking_mode = True
    assert session_manager.thinking_mode is True
    assert session_manager.secret_mode is False
    assert session_manager.search_mode is False
...
def test_session_manager_clear_chat_clears_search_mode(session_manager):
    """Test that clearing chat state also clears all modes."""
    session_manager.search_mode = True
    session_manager.secret_mode = True
    session_manager.thinking_mode = True
...
def test_session_manager_to_dict_includes_both_modes(session_manager):
    """Test that to_dict includes all modes."""
    session_manager.secret_mode = True
    session_manager.search_mode = True
    session_manager.thinking_mode = True
    ...
    assert "thinking_mode" in state_dict
    assert state_dict["secret_mode"] is True
    assert state_dict["search_mode"] is True
    assert state_dict["thinking_mode"] is True


def test_mode_combination_case1_both_persistent(session_manager):
    """Test Case 1: /secret on + /search on + /thinking on (all persistent)."""
    # Simulate /secret on
    session_manager.secret_mode = True
    # Simulate /search on
    session_manager.search_mode = True
    # Simulate /thinking on
    session_manager.thinking_mode = True
...
def test_unsupported_provider_with_thinking_mode(session_manager):
    """Test that thinking mode can be set even with unsupported provider (validation happens at use time)."""
    from poly_chat.models import provider_supports_thinking

    # Switch to unsupported provider
    session_manager._state.current_ai = "mistral"

    # Thinking mode can be set (validation happens when sending message)
    session_manager.thinking_mode = True
    assert session_manager.thinking_mode is True

    # But provider doesn't support thinking
    assert provider_supports_thinking(session_manager.current_ai) is False
