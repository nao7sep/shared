...
from revzip.errors import ExtractError, ZipIntegrityError
...
def test_restore_snapshot_keeps_existing_files_when_extract_fails(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "source"
    source_dir.mkdir()
    ...
    broken_zip = tmp_path / "broken.zip"
    with zipfile.ZipFile(broken_zip, mode="w") as zf:
        zf.writestr("dir", "plain file")
        zf.writestr("dir/file.txt", "nested file")
    ...
    with pytest.raises(ExtractError, match="Failed to extract zip archive"):
        restore_snapshot(source_dir_abs=source_dir, snapshot_record=snapshot_record)

    assert existing_file.read_text(encoding="utf-8") == "keep"
    assert not (source_dir / "dir").exists()
